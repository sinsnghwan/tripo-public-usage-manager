...
[v2.0.11 설정창 업데이트 내역]
- 설정창에 현재 확장 프로그램 버전 표시
- Supabase 최신 버전 표시
- Supabase의 버전별 업데이트 내역 목록 표시
- 업데이트 내역 새로고침 버튼 추가
- supabase-v2.0.11-release-notes.sql 1회 실행 필요
- 다음 버전부터 supabase-add-next-release-example.sql 형식으로 내역 추가 가능


[v2.0.12 전체 사용자 엑셀 내보내기]
- 본인 member_id 기록만 내보내던 문제 수정
- 현재 공유방의 전체 사용자 정상 사용 기록 포함
- 현재 공유방의 전체 계정 불일치 기록 포함
- 사용자 이름, 사용자 ID, 사용 일시, 작업 종류, 사용량, 계정 정보, 브라우저, 설치 ID, 세션 ID 포함
- 기록이 없으면 빈 헤더 파일 대신 안내 오류 표시
- supabase-v2.0.12-export-all.sql 1회 실행 필요
- 업데이트 내역 반영을 위해 supabase-v2.0.12-release.sql 실행
